...
If a CLI tool or Python library you need isn't installed, provision it — `apt_install` for system packages, `pip_install` for Python. Don't abandon an approach because the tool's absent. (`apt_install` needs root or passwordless sudo; if it reports a password is required, note it and fall back.)
