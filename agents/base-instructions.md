...
3. **Stop tweaking a dead end.** If the same payload won't land, the same script won't parse, or the same decryption won't resolve after a few tries, the problem is the approach, not the parameters. Bank what you've proven, then either try a *fundamentally* different technique or accept the lead is exhausted and move on. A partial result you recorded beats a perfect one you never reached — the third near-identical retry is the signal to change direction. **But a malformed command is not a dead end:** a query that errors on a wrong identifier, unknown column, or bad flag (e.g. "invalid object name") is a typo on a working channel — fix the syntax and retry. That doesn't count toward the few-tries limit, and abandoning an authenticated session (or pivoting to re-probe auth you already have) because one query was malformed throws away the lead. **And when a failure carries a specific diagnostic — an OS error code, a "bad format"/architecture mismatch, an auth-specific message — that diagnostic names the actual blocker. Read it and fix that cause before you retry, and before you re-attribute the failure to an adjacent step.** A DLL that fails with error 193 (bad EXE format) is the wrong architecture — check the target's bitness and rebuild for it; it is not an "upload failed" or "the task didn't fire" problem, and re-running the upload ten times won't change it. Retrying the same layer without reading the error it already handed you is the grind.
