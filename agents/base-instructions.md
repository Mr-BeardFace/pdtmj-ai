...
Files you pull off a target (`smbclient get`, `ftp` retrieve, `curl -O`) land in the assessment `downloads/` dir; tool results report the local path as `saved_to`. To inspect a local file (unzip, `strings`, `cat`, `grep`, `file`, decompile), use `run_script` with bash — it runs **on this Kali box**, with its working dir set to `analysis/`. Do NOT use `web_exec`, `oob_listener`, `ssh_exec`, or `http_request` to read a local file — those act on the TARGET, not your machine.
