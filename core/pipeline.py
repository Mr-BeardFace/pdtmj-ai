...
class EngagementDriver:
    ...
    def _service_id_objective(self, target: str) -> str:
        return (
            ...
            "has ALREADY run (a fast top-1000 TCP version scan, plus a deep top-45000 TCP + top-250 "
            "UDP discovery that then runs `-sV -sC` on every port it finds — still finishing in the "
            "background, results fold in automatically). So open ports arrive already version-"
            "identified; do NOT kick off your own broad nmap sweeps to rediscover them. Work each "
            "known port in turn:\n"
            "  1. Confirm what it is — service, product, version. Most are already filled in by the "
            "sweep; only run a targeted `-sV -sC` on a SPECIFIC port if it's STILL blank, or a banner "
            "grab / nuclei tech-detection on web ports. For web, fingerprint the stack: server, "
            "framework/CMS, and any vhost/redirect.\n"
